        ...
        if (!shoe.resellLinks.flightClub) {
            ...
            try {
                ...
                try {
                    ...
                    var lowestPrice = json.data.getProductTemplate.newSizes[0].lowestPriceOption.price.value / 100;
                    for (var i = 0; i < json.data.getProductTemplate.newSizes.length; i++) {
                        ...
                        if (lowestPrice > json.data.getProductTemplate.newSizes[i].lowestPriceOption.price.value / 100){
                            lowestPrice = json.data.getProductTemplate.newSizes[i].lowestPriceOption.price.value / 100;
                        }
                    }
                    shoe.resellPrices.flightClub = priceMap
                    shoe.lowestResellPrice.flightClub = lowestPrice;
                    callback(shoe);
                  } catch (error) {
                    let err = new Error("Could not connect to Flight Club while searching '" + shoe.styleID + "' Error: ", error)
                    console.log(err);
