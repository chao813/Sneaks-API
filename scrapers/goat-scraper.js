...
module.exports = {
  getLink: async function (shoe, callback) {
    ...
    try {
      const response = await got.get(
        "https://www.goat.com/_next/data/FgOruMvmcR-eL-bWID5Jc/en-US/search.json?query=" +
          shoe.styleID
      ); //https://ac.cnstrc.com/search/dunk?c=ciojs-client-2.26.6&key=key_XT7bjdbvjgECO5d8&i=fd9ba198-4864-4537-acdf-03e417886b13&s=2&num_results_per_page=25&_dt=1648919920096
      var json = JSON.parse(response.body);
      if (
        json.pageProps.constructorResponse.response.results[0].matched_terms[0]
      ) {
        if (
          json.pageProps.constructorResponse.response.results[0].data
            .lowest_price_cents /
            100 !=
          0
        ) {
        ...
        shoe.resellLinks.goat =
          "http://www.goat.com/sneakers/" +
          json.pageProps.constructorResponse.response.results[0].data.slug;
        shoe.goatProductId =
          json.pageProps.constructorResponse.response.results[0].data.id;
      }
      callback();
    } catch (error) {
      let err = new Error(
  ...
  getPrices: async function (shoe, callback) {
    if (!shoe.resellLinks.goat) {
      ...
      try {
        const response = await got.get(apiLink, {
          // this shit doesnt work? works in postman but not here
          headers: {
